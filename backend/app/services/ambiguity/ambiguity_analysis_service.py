...
class AmbiguityAnalysisService:
    def analyze(
        ...
        if top_score is None:
            return AmbiguityAnalysis(
                is_ambiguous=False,
                reason="Retrieval candidates do not contain reranker scores.",
                strong_candidate_count=0,
                distinct_document_count=0,
                top_score=None,
                second_score=None,
        ...
        strong_candidates = [
            ...
            for result in results
            if result.best_reranker_score is not None
            and result.best_reranker_score >= (top_score - 2.0)
        ]
        ...
        if query:
            responsibility_result = (
                responsibility_ambiguity_service.analyze_with_results(
                    query=query,
                    results=results,
                )
            )
        ...
        if responsibility_ambiguous:
            is_ambiguous = True
            reason = responsibility_result.reason

        elif retrieval_ambiguous:
            is_ambiguous = True
            reason = (
                "Multiple knowledge-base documents have similarly "
                "strong relevance scores, and the query does not "
                "explicitly identify which topic the user means."
            )

        elif explicit_topic:
            is_ambiguous = False
            reason = (
                "The query explicitly identifies its topic, so "
                "multiple relevant documents are treated as supporting "
                "evidence rather than unresolved ambiguity."
            )

        else:
            is_ambiguous = False
            reason = (
                "Retrieval evidence is sufficiently concentrated on "
                "one interpretation."
            )
...
ambiguity_analysis_service = AmbiguityAnalysisService()